<?php if ( ! defined('OC_ADMIN')) exit('Direct access is not allowed.');
    ...
    function customText($return = 'title'){
        ...
        $text = array();
        if( $new_item ) {
            $text['title']    = __('Anuncio');
            $text['subtitle'] = __('Agregar anuncio');
            $text['button']   = __('Agregar anuncio');
        } else {
            $text['title']    = __('Anuncio');
            $text['subtitle'] = __('Editar anuncio');
            $text['button']   = __('Actualizar anuncio');
        }
        return $text[$return];
    }
...
<div class="grid-row no-bottom-margin">
    <div class="row-wrapper">
        <h2 class="render-title"><?php echo customText('subtitle'); ?>  <span style="font-size: small;"><a href="<?php echo osc_item_url(); ?>"><?php _e('Ver el listado en el frente'); ?></a></span></h2>
    </div>
</div>
                ...
                <form action="<?php echo osc_admin_base_url(true); ?>" method="post" enctype="multipart/form-data" name="item">
                    ...
                    <div id="left-side">
                        ...
                        <div class="photo_container">
                            <label><?php _e('Fotos'); ?></label>
                            <?php ItemForm::photos(); ?>
                            <div id="photos">
                                <?php if( osc_max_images_per_item() == 0 || ( osc_max_images_per_item() != 0 && osc_count_item_resources() < osc_max_images_per_item() ) ) { ?>
                                <div>
                                    <input type="file" name="photos[]" /> (<?php _e('opcional'); ?>)
                                </div>
                                <?php } ?>
                            </div>
                            <p><a href="#" onclick="addNewPhoto(); return false;"><?php _e('Agregar nueva foto'); ?></a></p>
                        </div>
                        <?php } ?>
                        <?php if( $new_item ) {
                    ...
                    <div id="right-side">
                        <div class="well ui-rounded-corners">
                            <h3 class="label"><?php _e('Usuario'); ?></h3>
                            <div id="contact_info">
                        ...
                        <div class="well ui-rounded-corners input-separate-top">
                            <h3 class="label"><?php _e('Ubicacion'); ?></h3>
                            <div class="input-has-placeholder input-separate-top">
                                <label><?php _e('Pais'); ?></label>
                            ...
                            <div class="input-has-placeholder input-separate-top">
                                <label><?php _e('Ciudad'); ?></label>
                                <?php ItemForm::city_text(); ?>
                            </div>
                            <div class="input-has-placeholder input-separate-top">
                                <label><?php _e('Area urbana'); ?></label>
                            ...
                            <div class="input-has-placeholder input-separate-top">
                                <label><?php _e('Direccion'); ?></label>
                                <?php ItemForm::address_text(); ?>
                            </div>
                        </div>

                        <div class="well ui-rounded-corners input-separate-top">
                            <h3 class="label"><?php _e('Expiracion'); ?></h3>
                            <?php if( $new_item ) { ?>
                                <div class="input-has-placeholder input-separate-top">
