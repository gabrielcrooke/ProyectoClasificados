<?php if ( ! defined('OC_ADMIN')) exit('Direct access is not allowed.');
    ...
    function customPageHeader() { ?>
        <h1><?php _e('Anuncios'); ?>
            <a href="<?php echo osc_admin_base_url(true) . '?page=items&action=post'; ?>" class="btn btn-green ico ico-32 ico-add-white float-right"><?php _e('Agregar anuncios'); ?></a>
    </h1>
<?php
    }
...
<h2 class="render-title"><?php _e('Gestionar anuncios'); ?> <a href="<?php echo osc_admin_base_url(true) . '?page=items&action=post'; ?>" class="btn btn-mini"><?php _e('Agregar nuevo'); ?></a></h2>
<div class="relative">
        ...
        <div class="float-right">
            <form method="get" action="<?php echo osc_admin_base_url(true); ?>"  class="inline nocsrf">
                ...
                <select name="iDisplayLength" class="select-box-extra select-box-medium float-left" onchange="this.form.submit();" >
                    <option value="10"><?php printf(__('%d Anuncios'), 10); ?></option>
                    <option value="25" <?php if( Params::getParam('iDisplayLength') == 25 ) echo 'selected'; ?> ><?php printf(__('%d Anuncios'), 25); ?></option>
                    <option value="50" <?php if( Params::getParam('iDisplayLength') == 50 ) echo 'selected'; ?> ><?php printf(__('%d Anuncios'), 50); ?></option>
                    <option value="100" <?php if( Params::getParam('iDisplayLength') == 100 ) echo 'selected'; ?> ><?php printf(__('%d Anuncios'), 100); ?></option>
                </select>
            </form>
            <form method="get" action="<?php echo osc_admin_base_url(true); ?>" id="shortcut-filters" class="inline">
                ...
                <select id="filter-select" name="shortcut-filter" class="select-box-extra select-box-input">
                    <option value="oPattern" <?php if($opt == 'oPattern'){ echo 'selected="selected"'; } ?>><?php _e('Patron'); ?></option>
                    <option value="oUser" <?php if($opt == 'oUser'){ echo 'selected="selected"'; } ?>><?php _e('Email'); ?></option>
                    <option value="oItemId" <?php if($opt == 'oItemId'){ echo 'selected="selected"'; } ?>><?php _e('Articulo ID'); ?></option>
                </select><input
                ...
                <input type="submit" class="btn submit-right" value="<?php echo osc_esc_html( __('Buscar') ); ?>">
            </form>
        </div>
    </div>
    <form class="" id="datatablesForm" action="<?php echo osc_admin_base_url(true); ?>" method="post" data-dialog-open="false">
        ...
        <div id="bulk-actions">
            <label>
                ...
                <input type="submit" id="bulk_apply" class="btn" value="<?php echo osc_esc_html( __('Aplicar') ); ?>" />
            </label>
        </div>
            ...
            <table class="table" cellpadding="0" cellspacing="0">
                ...
                <?php } else { ?>
                    <tr>
                        ...
                        <p><?php _e('No hay datos disponibles en la tabla'); ?></p>
                        </td>
                    </tr>
                <?php } ?>
                </tbody>
...
<form id="dialog-item-delete" method="get" action="<?php echo osc_admin_base_url(true); ?>" class="has-form-actions hide">
    ...
    <div class="form-horizontal">
        <div class="form-row">
            <?php echo osc_apply_filter('admin_dialog_delete_listing_text', __('¿Estás seguro de que quieres eliminar este anuncio?')); ?>
        </div>
        <div class="form-actions">
            <div class="wrapper">
            <a class="btn" href="javascript:void(0);" onclick="$('#dialog-item-delete').dialog('close');"><?php _e('Cancelar'); ?></a>
            <input id="item-delete-submit" type="submit" value="<?php echo osc_esc_html( __('Delete') ); ?>" class="btn btn-red" />
            </div>
        </div>
    </div>
</form>
<div id="dialog-bulk-actions" title="<?php _e('Acciones'); ?>" class="has-form-actions hide">
    <div class="form-horizontal">
        <div class="form-row"></div>
        <div class="form-actions">
            <div class="wrapper">
                <a id="bulk-actions-cancel" class="btn" href="javascript:void(0);"><?php _e('Cancelar'); ?></a>
                <a id="bulk-actions-submit" href="javascript:void(0);" class="btn btn-red" ><?php echo osc_esc_html( __('Borrar') ); ?></a>
                <div class="clear"></div>
            </div>
        </div>
    </div>
