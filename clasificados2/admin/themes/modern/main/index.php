<?php if ( ! defined('OC_ADMIN')) exit('Direct access is not allowed.');
    ...
    function customPageHeader() { ?>
        <h1><?php _e('Panel de administracion'); ?></h1>
    <?php
    }
    ...
    function customHead() {
        ...
        <script type="text/javascript">
            ...
            function drawChartUser() {
                var data = new google.visualization.DataTable();
                data.addColumn('string', '<?php _e('Fecha'); ?>');
                data.addColumn('number', '<?php _e('Usuarios'); ?>');
                data.addColumn({type:'boolean',role:'certainty'});
                <?php $k = 0;
...
<div class="grid-system">
    <div class="grid-row grid-first-row grid-50">
        <div class="row-wrapper">
			...
			<center>
                <div class="widget-box-title"><h3><?php _e('Anuncios por categorias'); ?></h3></div>
                <div class="widget-box-content">
                    ...
                    <table class="table" cellpadding="0" cellspacing="0">
                        ...
                        foreach($numItemsPerCategory as $c) {?>
                            <tr<?php if($even == true){ $even = false; echo ' class="even"'; } else { $even = true; } if($countEvent == 1){ echo ' class="table-first-row"';} ?>>
                                ...
                                <td><?php echo $c['i_num_items'] . "&nbsp;" . ( ( $c['i_num_items'] == 1 ) ? __('Listing') : __('Anuncios') ); ?></td>
                            </tr>
                            <?php foreach($c['categories'] as $subc) {?>
                                <tr<?php if($even == true){ $even = false; echo ' class="even"'; } else { $even = true; } ?>>
                                    ...
                                    <td><?php echo $subc['i_num_items'] . " " . ( ( $subc['i_num_items'] == 1 ) ? __('Listing') : __('Anuncios') ); ?></td>
                                </tr>
                            <?php
                            $countEvent++;
                            }
                    ...
                    <?php } else { ?>
                        <?php _e("No hay ningún anuncio subido todavía"); ?>
                    <?php } ?>
                </div>
            </div>
        </div>
    ...
    <div class="grid-row grid-50">
        <div class="row-wrapper">
            <div class="widget-box">
                <div class="widget-box-title"><h3><?php _e('Estadisticas'); ?> <select id="widget-box-stats-select" class="widget-box-selector select-box-big input-medium"><option value="listing"><?php _e('Nuevos anuncios'); ?></option><option value="users"><?php _e('Nuevos usuarios'); ?></option></select></h3></div>
                <div class="widget-box-content widget-box-content-stats" style="overflow-y: visible;">
                    <div id="widget-box-stats-listings" class="widget-box-stats">
                        <b class="stats-title"><?php _e('Nuevos anuncios'); ?></b>
                        <div class="stats-detail"><?php printf(__('Total numero de anuncios: %s'), $numItems); ?></div>
                        <div id="placeholder-listing" class="graph-placeholder"></div>
                        <a href="<?php echo osc_admin_base_url(true); ?>?page=stats&amp;action=items" class="btn"><?php _e('Estadisticas de anuncios'); ?></a>
                    </div>
                    <div id="widget-box-stats-users" class="widget-box-stats" style="visibility: hidden;">
                        <b class="stats-title"><?php _e('Nuevos usuarios'); ?></b>
                        <div class="stats-detail"><?php printf(__('Total numero de usuarios: %s'), $numUsers); ?></div>
