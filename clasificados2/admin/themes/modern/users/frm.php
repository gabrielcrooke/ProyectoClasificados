<?php if ( ! defined('OC_ADMIN')) exit('Direct access is not allowed.');
    ...
    function customFrmText(){
        ...
        if( isset($user['pk_i_id']) ) {
            $return['edit']       = true;
            $return['title']      = __('Editar usuario');
            $return['action_frm'] = 'edit_post';
            $return['btn_text']   = __('Actualizar usuario');
...
<div class="clear"></div>
    ...
    <div class="settings-user">
        ...
        <form name="register" action="<?php echo osc_admin_base_url(true); ?>" method="post">
            ...
            <h3 class="render-title"><?php _e('Informacion del contacto'); ?></h3>
            ...
            <div class="form-horizontal">
                ...
                <?php }; ?>
                <div class="form-row">
                    <div class="form-label"><?php _e('Nombre'); ?></div>
                    <div class="form-controls">
                        <?php UserForm::name_text($user); ?>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-label"><?php _e('Usuario'); ?></div>
                    <div class="form-controls">
                        <?php UserForm::username_text($user); ?> <div id="available"></div>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-label"><?php _e('E-mail'); ?> <em><?php _e('(requerido)'); ?></em></div>
                    <div class="form-controls">
                        <?php UserForm::email_text($user); ?>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-label"><?php _e('Celular'); ?></div>
                    <div class="form-controls">
                        <?php UserForm::mobile_text($user); ?>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-label"><?php _e('Telefono'); ?></div>
                    <div class="form-controls">
                        <?php UserForm::phone_land_text($user); ?>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-label"><?php _e('Sitio web'); ?></div>
                    <div class="form-controls">
                        <?php UserForm::website_text($user); ?>
                    </div>
                </div>
                <h3 class="render-title"><?php _e('Sobre usted'); ?></h3>
                <div class="form-row">
                    <div class="form-label"><?php _e('Tipo de usuario'); ?></div>
                    <div class="form-controls">
                        <?php UserForm::is_company_select($user); ?>
                    </div>
                </div>
                
                <h3 class="render-title"><?php _e('Ubicacion'); ?></h3>
                <div class="form-row">
                    <div class="form-label"><?php _e('Pais'); ?></div>
                    <div class="form-controls">
                ...
                <div class="form-row">
                    <div class="form-label"><?php _e('Ciudad'); ?></div>
                    <div class="form-controls">
                        <?php UserForm::city_select($cities, $user); ?>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-label"><?php _e('Zona urbana'); ?></div>
                    <div class="form-controls">
                ...
                <div class="form-row">
                    <div class="form-label"><?php _e('Direccion'); ?></div>
                    <div class="form-controls">
                        <?php UserForm::address_text($user); ?>
                    </div>
                </div>
                <h3 class="render-title"><?php _e('Contraseña'); ?></h3>
                <div class="form-row">
                    <div class="form-label"><?php _e('Nueva contraseña'); ?><?php if(!$aux['edit']) { printf('<br/><em>%s</em>', __('(twice, required)')); } ?></div>
                    <div class="form-controls">
                        <?php UserForm::password_text($user); ?>
                        <?php if($aux['edit']) { ?>
                            <p class="help-inline"><?php _e("Si desea cambiar la contraseña, escriba una nueva. De lo contrario, deje este espacio en blanco"); ?></p>
                        <?php } ?>
                        <div class="input-separate-top">
                            <?php UserForm::check_password_text($user); ?>
                            <?php if($aux['edit']) { ?>
                                <p class="help-inline"><?php _e('Escribe tu nueva contraseña nuevamente'); ?></p>
                            <?php } ?>
                        </div>
                    </div>
                </div>
    ...
    <?php if($aux['edit'] && count($aux['alerts'])>0) { ?>
                    ...
                    <div class="form-row">
                        <?php for($k=0;$k<count($aux['alerts']);$k++) { 
                            ...
                            <div class="form-label">
                                ...
                                <a href="javascript:delete_alert('<?php echo $aux['alerts'][$k]['pk_i_id']; ?>');" ><?php _e("Borrar"); ?></a>
                                &nbsp;|&nbsp;
        ...
        <form id="dialog-alert-delete" method="get" action="<?php echo osc_admin_base_url(true); ?>" class="has-form-actions hide" title="<?php echo osc_esc_html(__('Delete alert')); ?>">
            ...
            <div class="form-horizontal">
                ...
                <div class="form-actions">
                    <div class="wrapper">
                    <a class="btn" href="javascript:void(0);" onclick="$('#dialog-alert-delete').dialog('close');"><?php _e('Cancelar'); ?></a>
                    <input id="alert-delete-submit" type="submit" value="<?php echo osc_esc_html( __('Borrar') ); ?>" class="btn btn-red" />
                    </div>
                </div>
            </div>
        </form>
